...
/**
 * Custom adapter class for displaying all order items in a ListView.
 * Extends {@link BaseAdapter} to handle list of {@link MenuItem} objects.
 *
 * @author Soham Patel
 * @version 1.0
 */
public class CustomAdapterAllOrders extends BaseAdapter {
    ...
    /**
     * Constructs a new CustomAdapterAllOrders.
     *
     * @param context The current context.
     * @param items The list of MenuItems that the adapter will manage.
     */
    public CustomAdapterAllOrders(Context context, List<MenuItem> items) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.current_order_item_list, parent, false);
        }

        TextView textView = convertView.findViewById(R.id.currentOrderItemList);
        MenuItem item = items.get(position);
        textView.setText(item.toString());  // Ensure your MenuItem class has a proper toString() implementation
